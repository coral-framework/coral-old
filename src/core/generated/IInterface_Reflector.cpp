...
public:
	...
	{
		_cookie = _provider->registerProxyInterface( co::disambiguate<co::IService, co::IInterface>( this ) );
	}
	...
	co::IInterface* getInterfaceType() { return co::typeOf<co::IInterface>::get(); }
	...
	{
		const co::Any& res = _provider->handleGetAttribute( _cookie, getAttribInfo<co::IInterface>( 0 ) );
        return res.get< const std::string& >();
	}

	co::Range<co::IInterface* const> getInterfaceAncestors()
	{
		const co::Any& res = _provider->handleGetAttribute( _cookie, getAttribInfo<co::IInterface>( 1 ) );
        return res.get< co::Range<co::IInterface* const> >();
	}

	co::Range<co::IInterface* const> getSubInterfaces()
	{
		const co::Any& res = _provider->handleGetAttribute( _cookie, getAttribInfo<co::IInterface>( 2 ) );
        return res.get< co::Range<co::IInterface* const> >();
	}

	co::Range<co::IInterface* const> getSuperInterfaces()
	{
		const co::Any& res = _provider->handleGetAttribute( _cookie, getAttribInfo<co::IInterface>( 3 ) );
        return res.get< co::Range<co::IInterface* const> >();
	}
	...
	bool isSubTypeOf( co::IInterface* itf_ )
	{
		...
		co::Range<co::Any const> range( args, 1 );
		const co::Any& res = _provider->handleMethodInvocation( _cookie, getMethodInfo<co::IInterface>( 0 ), range );
		return res.get< bool >();
	}
...
public:
	...
	{
		co::IInterface* p = checkInstance( instance, ai );
		switch( ai->getIndex() )
		{
	...
	{
		co::IInterface* p = checkInstance( instance, mi );
