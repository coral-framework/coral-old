...
public:
	...
	co::Interface* newProxy( co::DynamicProxyHandler* handler )
	{
		...
		return co::disambiguate<co::Interface, co::CompoundType>( new CompoundType_Proxy( handler ) );
	}
			...
			case 1:
				{
					...
					argIndex = -1;
					__res.set< co::MemberInfo* >( p->getMember( name ) );
				}
				break;
			default:
...
private:
	co::CompoundType* checkInstance( const co::Any& instance, co::MemberInfo* member )
	{
		...
		return reinterpret_cast<co::CompoundType*>( instance.getState().data.ptr );
	}
};
