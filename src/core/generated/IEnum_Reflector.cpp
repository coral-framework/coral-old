...
public:
	...
	{
		_cookie = _provider->registerProxyInterface( co::disambiguate<co::IService, co::IEnum>( this ) );
	}
	...
	co::IInterface* getInterfaceType() { return co::typeOf<co::IEnum>::get(); }
	...
	{
		const co::Any& res = _provider->handleGetAttribute( _cookie, getAttribInfo<co::IEnum>( 0 ) );
	...
	co::int32 getValueOf( const std::string& identifier_ )
	{
		...
		const co::Any& res = _provider->handleMethodInvocation( _cookie, getMethodInfo<co::IEnum>( 0 ), range );
...
public:
	...
	{
		co::IEnum* p = checkInstance( instance, ai );
		switch( ai->getIndex() )
		{
	...
	{
		co::IEnum* p = checkInstance( instance, mi );
