...
public:
	...
	{
		_cookie = _provider->registerProxyInterface( co::disambiguate<co::IService, co::IClassType>( this ) );
	}
	...
	co::IInterface* getInterfaceType() { return co::typeOf<co::IClassType>::get(); }
...
public:
	...
	co::IService* newProxy( co::IDynamicServiceProvider* provider )
	{
		...
		return co::disambiguate<co::IService, co::IClassType>( new co::IClassType_Proxy( provider ) );
	}

	void getAttribute( const co::Any& instance, co::IField* ai, co::Any& value )
	{
		co::IClassType* p = checkInstance( instance, ai );
		switch( ai->getIndex() )
		{
...
private:
	co::IClassType* checkInstance( const co::Any& any, co::IMember* member )
	{
		if( !member )
			throw co::IllegalArgumentException( "illegal null member info" );

		// make sure that 'any' is an instance of this type
		co::IInterface* myType = co::typeOf<co::IClassType>::get();

		co::IClassType* res;
		if( any.getKind() != co::TK_INTERFACE || !( res = dynamic_cast<co::IClassType*>( any.getState().data.itf ) ) )
			CORAL_THROW( co::IllegalArgumentException, "expected a valid co::IClassType*, but got " << any );

		// make sure that 'member' belongs to this type
		co::ICompositeType* owner = member->getOwner();
		if( owner != myType )
			CORAL_THROW( co::IllegalArgumentException, "member '" << member->getName() << "' belongs to "
				<< owner->getFullName() << ", not to co.IClassType" );
