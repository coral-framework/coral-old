...
public:
	...
	{
		co::Any __res;
		_handler->handleGetAttribute( _cookie, getAttribInfo<co::NativeClassType>( 0 ), __res );
        return __res.get< const std::string& >();
	}
	...
	{
		co::Any __res;
		_handler->handleGetAttribute( _cookie, getAttribInfo<co::NativeClassType>( 1 ), __res );
        return __res.get< const std::string& >();
	}
...
public:
	...
	co::Interface* newProxy( co::DynamicProxyHandler* handler )
	{
		...
		return co::disambiguate<co::Interface, co::NativeClassType>( new NativeClassType_Proxy( handler ) );
	}
	...
	void getAttribute( const co::Any& instance, co::AttributeInfo* ai, co::Any& value )
	{
		...
		switch( ai->getIndex() )
		{
		case 0:		value.set< const std::string& >( p->getHeaderName() ); break;
...
private:
	co::NativeClassType* checkInstance( const co::Any& instance, co::MemberInfo* member )
	{
		...
		return reinterpret_cast<co::NativeClassType*>( instance.getState().data.ptr );
	}
};
